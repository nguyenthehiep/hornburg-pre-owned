...
class common_util {
	...
	public static function load_slider_info ($vindb, $detail_id){		
		if(empty($detail_id)){
			$field = array ("ListingID", "ModelYear", "Make", "Model", "Trim", "SalePrice", "CarImageUrl");
			
			//44674: Hornburg preowned 
			$table1 = $vindb-> select();			
			$table1->from ( "whitmanenterprisedealershipinventory", $field);			
			$table1->where("whitmanenterprisedealershipinventory.DealershipId = ?","44674");
			
			//1541 : Newport beach land rover and jaguar
			$table2 = $vindb-> select();
			$table2->from ( "whitmanenterprisedealershipinventory", $field );			
			$table2->where("whitmanenterprisedealershipinventory.DealershipId = ?","1541");
			$table2->where("NewUsed = ?","Used");
			
			//2584 : Mission Viejo Land Rover and jaguar
			$table3 = $vindb-> select();
			$table3->from ( "whitmanenterprisedealershipinventory", $field );			
			$table3->where("whitmanenterprisedealershipinventory.DealershipId = ?","2584");
			$table3->where("NewUsed = ?","Used");
			
			//23063: Jaguar and Land rover los angles
			$table4 = $vindb-> select();
			$table4->from ( "whitmanenterprisedealershipinventory", $field );			
			$table4->where("whitmanenterprisedealershipinventory.DealershipId = ?","23063");
			$table4->where("NewUsed = ?","Used");
			
			//1563: Jaguar and land rover santa monica			
			$table5 = $vindb-> select();
			$table5->from ( "whitmanenterprisedealershipinventory", $field );			
			$table5->where("whitmanenterprisedealershipinventory.DealershipId = ?","1563");
			$table5->where("NewUsed = ?","Used");
			
			$sql = $vindb-> select();
			$sql ->union(array($table1, $table2, $table3, $table4, $table5));
			
		}else{
			$sql = $vindb-> select();	
			$sql->from ( "whitmanenterprisedealershipinventory", array (
	...
	public static function load_features_info ($vindb){
		$field = array ("ListingID", "ModelYear", "Make", "Model", "Trim",
			"VINNumber", "StockNumber",
			"ExteriorColor", "InteriorColor",
...
//var_dump("__toString($sql)");
	...
	public function inventory_pull_data($vindb,$make, $model, $trim, $year, $price, $underPrice) {
		...
		if(!empty($underPrice)){
			switch($underPrice){
				case "u10000":
					$table1->where ("SalePrice <= ?","10000");
					$table2->where ("SalePrice <= ?","10000");
					$table3->where ("SalePrice <= ?","10000");
					$table4->where ("SalePrice <= ?","10000");
					$table5->where ("SalePrice <= ?","10000");
					
					break;
				case "u15000":
					$table1->where ("SalePrice <= ?","15000");
					$table2->where ("SalePrice <= ?","15000");
					$table3->where ("SalePrice <= ?","15000");
					$table4->where ("SalePrice <= ?","15000");
					$table5->where ("SalePrice <= ?","15000");					
					break;
				case "u20000":
					$table1->where ("SalePrice <= ?","20000");
					$table2->where ("SalePrice <= ?","20000");
					$table3->where ("SalePrice <= ?","20000");
					$table4->where ("SalePrice <= ?","20000");
					$table5->where ("SalePrice <= ?","20000");
					break;
				case "u25000":
					$table1->where ("SalePrice <= ?","25000");
					$table2->where ("SalePrice <= ?","25000");
					$table3->where ("SalePrice <= ?","25000");
					$table4->where ("SalePrice <= ?","25000");
					$table5->where ("SalePrice <= ?","25000");
					break;
				default:
			}
		}else{
				if(!empty($make)){
					$table1->where ("Make = ?","$make");
					$table2->where ("Make = ?","$make");
					$table3->where ("Make = ?","$make");
					$table4->where ("Make = ?","$make");
					$table5->where ("Make = ?","$make");
				}
				...
				if (!empty($model)){
					if($model == "Not Specified"){
						...
						$table1->where ("Model = ?","$model");
						$table2->where ("Model = ?","$model");
						$table3->where ("Model = ?","$model");
						$table4->where ("Model = ?","$model");
						$table5->where ("Model = ?","$model");
					}
				...
				if(!empty($trim)){
					if($trim == "Not Specified"){
						...
						$table1->where ("Trim = ?","$trim");	
						$table2->where ("Trim = ?","$trim");
						$table3->where ("Trim = ?","$trim");
						$table4->where ("Trim = ?","$trim");
						$table5->where ("Trim = ?","$trim");						
					}			
				}
		
				if(!empty($year)){
					$table1->where ("ModelYear = ?","$year");
					$table2->where ("ModelYear = ?","$year");
					$table3->where ("ModelYear = ?","$year");
					$table4->where ("ModelYear = ?","$year");
					$table5->where ("ModelYear = ?","$year");
				}
		
				if(!empty($price)){			
					$table1->where ("SalePrice = ?","$price");
					$table2->where ("SalePrice = ?","$price");
					$table3->where ("SalePrice = ?","$price");
					$table4->where ("SalePrice = ?","$price");
					$table5->where ("SalePrice = ?","$price");
				}
		}
